...
const YearSelection = () => {
  ...
  const years = [
    ...
    },
    {
      ...
      description: "Electives, projects, and placement prep",
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-gray-50 dark:from-gray-900 dark:to-gray-950">
      <Seo
        title="Choose Your Year — AKTU B.Tech Resources"
        description="Select your academic year to access notes, PYQs, CAE papers and question banks for all subjects."
        path="/year-selection"
      />
      <Navigation />
      <main id="main-content" className="container mx-auto px-4 pt-32 pb-20">
        ...
        <p className="text-lg text-gray-600 dark:text-gray-300 text-center mb-8 max-w-2xl mx-auto">
          Select your academic year to access relevant study materials, notes,
          and resources
        </p>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
          {years.map((item, index) => (
            <button
              key={item.year}
              onClick={() =>
                navigate(`/resources/${item.year.split(" ")[0].toLowerCase()}`)
              }
            ...
            >
              <div
                className={`w-16 h-16 rounded-xl ${item.bgColor} ${item.color} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform`}
              >
                <item.icon size={32} />
              </div>
